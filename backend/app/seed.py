...
async def seed():
    ...
    async with async_session() as db:
        ...
        settings = [
            ...
            Setting(key="service_areas", value=[
                {"type": "country", "name": "United States", "country": "US"},
            ], description="Service areas where bookings can be made. Each entry: {type:'country'|'city', country, name, [bounds, lat, lng, place_id]}. Configurable from admin Settings page."),
            ...
            Setting(key="min_advance_booking_hours", value=0.5, description="Minimum hours before pickup that a client can book. Supports decimals (0.5 = 30 min, 1.5 = 1h 30m)."),
            ...
            Setting(key="sms_guest_payment_link", value="Hi {client_name}, a ride has been reserved for you by {cashier_name}: {pickup_name} → {dropoff_name} on {pickup_date} at {pickup_time}. Total: ${total_amount}. Pay here: {payment_url}", description="SMS to guest when cashier books for them. Vars: {client_name}, {cashier_name}, {hotel_name}, {pickup_name}, {dropoff_name}, {pickup_date}, {pickup_time}, {total_amount}, {payment_url}, {booking_number}"),
            ...
            Setting(key="sms_client_refund", value="Hi {client_name}, your booking {booking_number} has been refunded (${amount}) — {reason}. The refund has been issued to your card right away.", description="SMS to client on refund. Vars: {client_name}, {booking_number}, {amount}, {reason}"),
