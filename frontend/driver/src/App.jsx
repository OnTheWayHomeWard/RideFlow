...
export default function App() {
  ...
  useEffect(() => { api.getPublicSettings().then(setSettings).catch(() => {}) }, [])

  const login = (accessToken, name, passwordChanged) => {
    localStorage.setItem('driver_token', accessToken)
    localStorage.setItem('driver_name', name)
    localStorage.setItem('driver_pw_changed', passwordChanged ? 'true' : 'false')
    setToken(accessToken); setPwChanged(passwordChanged)
  }
  const logout = () => {
    localStorage.removeItem('driver_token'); localStorage.removeItem('driver_name'); localStorage.removeItem('driver_pw_changed')
    setToken(null)
  }
  ...
  if (!token) {
    return (
      <BrowserRouter><Routes>
        <Route path="/login" element={<Login onLogin={login} settings={settings} />} />
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        {/* Password banner */}
        {!pwChanged && (
          <NavLink to="/change-password" className="bg-amber-50 border-b border-amber-200 px-4 py-2.5 flex items-center gap-2 hover:bg-amber-100">
        ...
        <nav className="bg-white border-t border-slate-200 px-2 py-2 flex justify-around sticky bottom-0">
          {[
            { to: '/', label: 'Runs', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', end: true },
