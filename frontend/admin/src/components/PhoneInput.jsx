...
function parsePhone(phoneValue, countryList) {
  if (!phoneValue) return { country: countryList[0] || COUNTRIES[0], local: '' }
  ...
  return { country: countryList[0] || COUNTRIES[0], local: phoneValue.replace(/^\+\d{1,3}/, '') }
}
...
export default function PhoneInput({ value, onChange, availableCountries, placeholder }) {
  ...
  const parsed = parsePhone(value, allowed)
  const [selected, setSelected] = useState(parsed.country)
  const [localNumber, setLocalNumber] = useState(parsed.local)
  const [open, setOpen] = useState(false)
  const [initialized, setInitialized] = useState(false)

  // Re-parse when value or availableCountries change (async load)
  useEffect(() => {
    if (allowed.length === 0) return
    const p = parsePhone(value, allowed)
    setSelected(p.country)
    setLocalNumber(p.local)
    setInitialized(true)
  }, [availableCountries, initialized ? null : value])
