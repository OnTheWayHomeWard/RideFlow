      ...
      {loading ? (
        ...
        <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {bookings.map(b => (
                    <tr key={b.id} onClick={() => window.location.href = `/admin/runs/${b.id}`} className={`hover:bg-slate-50 cursor-pointer ${b.client_rating && b.client_rating <= 2 ? 'bg-red-50/50' : ''}`}>
          ...
          <div className="md:hidden space-y-3">
            {bookings.map(b => (
              <Link key={b.id} to={`/admin/runs/${b.id}`} className={`block bg-white border rounded-xl overflow-hidden hover:shadow-sm transition-all ${b.client_rating && b.client_rating <= 2 ? 'border-red-200' : 'border-slate-200'}`}>
                <div className="p-4">
